"""
Team assignment via CLIP-based jersey-colour classification.

Each player's bounding-box crop is passed through a fashion-CLIP model that
scores it against two text descriptions (one per team).  Results are cached
per player ID and periodically refreshed every 50 frames to account for
...
class TeamAssigner:
    ...
    def get_player_teams_across_frames(
        ...
        for frame_num, player_track in enumerate(player_tracks):
            ...
            # Refresh the per-player cache every 50 frames to handle
            # appearance changes from lighting or camera angles.
            if frame_num % 50 == 0:
                self.player_team_dict = {}
