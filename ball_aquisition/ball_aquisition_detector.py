"""
Ball possession detection using spatial heuristics.

For each frame the detector finds the player whose bounding box best
"contains" the ball, falling back to a minimum-distance check when
containment is low.  A consecutive-frame requirement prevents spurious
single-frame assignments from being reported as possession changes.
"""
...
class BallAquisitionDetector:
    ...
    def get_key_basketball_player_assignment_points(self, player_bbox, ball_center):
        ...
        # Dynamic edge-projection points (only when the ball is within range)
        ...
        # Fixed reference points around the bounding box
        output_points += [
    ...
    def find_best_candidate_for_possession(self, ball_center, player_tracks_frame, ball_bbox):
        ...
        # Priority 1: players with high ball containment
        if high_containment_players:
            best_candidate = max(high_containment_players, key=lambda x: x[1])
            return best_candidate[0]

        # Priority 2: closest player within the distance threshold
